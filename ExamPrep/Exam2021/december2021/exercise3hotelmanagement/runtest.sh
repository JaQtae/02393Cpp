...
Extra=$?
if [ ${Extra} -eq 0 ]; then
	run_test Extra Extra
	Extra=$?
fi

print_feedback Example TaskA TaskB TaskC Extra
compute_score Example TaskA TaskB TaskC Extra
print_feedback Example TaskA TaskB TaskC Extra > ../results.txt
compute_score Example TaskA TaskB TaskC Extra >> ../results.txt
cd ..
if test -f tests/$Library; then
	rm tests/$Library
fi
